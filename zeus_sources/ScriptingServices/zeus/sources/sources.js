...
var sourcesDao = require('zeus/sources/dao/sourcesDao');
...
function handleGetRequest(httpRequest, httpResponse, xss) {
    ...
    if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
        if (id) {
            var entity = sourcesDao.get(id);
            if (entity !== null) {
                sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
            } else {
                sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'scm_id\'=' + id);
            }
        } else if (count !== null) {
            var sourcesCount = sourcesDao.count();
            sendResponse(httpResponse, httpResponse.OK, 'text/plain', sourcesCount);
        } else if (metadata !== null) {
            var sourcesMetadata = sourcesDao.metadata();
            sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(sourcesMetadata, null, 2));
        } else {
            var sources = sourcesDao.list(limit, offset, sort, desc);
            sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(sources, null, 2));
        }
    }
}
...
function handlePostRequest(httpRequest, httpResponse) {
    ...
    var id = sourcesDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
    var entity = getRequestBody(httpRequest);
    var id = getIdParameter(httpRequest, xss);
    id = id !== null ? id : entity.scm_id;
    if (id !== null) {
        if (sourcesDao.get(id) !== null) {
            sourcesDao.update(entity);
            sendResponse(httpResponse, httpResponse.NO_CONTENT);
        } else {
            sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'scm_id\'=' + id);
        }
    } else {
        sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'scm_id\' parameter is missing!');
    }
...
function getIdParameter(httpRequest, xss) {
    ...
    id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('scm_id'));
    return id;
}
