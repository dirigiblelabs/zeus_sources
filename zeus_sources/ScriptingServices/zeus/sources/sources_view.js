...
function handleGetRequest(httpRequest, httpResponse, xss) {
	...
	if (!hasConflictingParameters(null, count, metadata, httpResponse)) {
		if (count !== null) {
			var sources_viewCount = sources_viewDao.count();
			sendResponse(httpResponse, httpResponse.OK, 'text/plain', sources_viewCount);
		} else if (metadata !== null) {
			var sources_viewMetadata = sources_viewDao.metadata();
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(sources_viewMetadata, null, 2));
		} else {
			var sources_view = sources_viewDao.list(limit, offset, sort, desc);
			sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(sources_view, null, 2));
		}
	}
}
